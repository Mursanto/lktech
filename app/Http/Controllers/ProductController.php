...
class ProductController extends Controller
{
    ...
    {
        $categories = \App\Models\Category::with('children')->whereNull('parent_id')->get();
        $investors = \App\Models\Investor::where('is_active', true)->orderBy('name')->get();
        $autoGeneratedId = 'SKU-' . date('YmdHis') . '-' . rand(1000, 9999);
        return view('products.create', compact('categories', 'autoGeneratedId', 'investors'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->all();
        $data['status'] = $status;
        $data['ownership_type'] = $request->ownership_type ?? 'lktech';
        $data['investor_id']    = ($request->ownership_type === 'investor') ? $request->investor_id : null;
    ...
    public function edit(Product $product)
    {
        $categories = \App\Models\Category::with('children')->whereNull('parent_id')->get();
        $investors = \App\Models\Investor::where('is_active', true)->orderBy('name')->get();
        return view('products.edit', compact('product', 'categories', 'investors'));
    }
