<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            <div>
                <h2 class="text-base font-bold text-natural-900 tracking-tight leading-none">Informasi Laba &amp; Rugi</h2>
                <p class="text-natural-500 text-[10px] mt-0.5">Ringkasan pendapatan dari Penjualan, Service, dan Sewa secara real-time.</p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('reports.profit-loss.export') ?? '#' }}" class="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600 rounded-lg text-white text-[11px] font-bold hover:bg-rose-700 transition-all shadow-sm">
    ...
    <div class="flex flex-col h-full space-y-4">
        ...
        <form action="{{ route('reports.index') }}" method="GET">
            <div class="bg-white rounded-2xl border border-natural-100 shadow-sm px-4 py-2.5 flex flex-wrap items-end gap-3">
                <p class="text-[9px] font-bold text-natural-400 uppercase tracking-wider flex items-center gap-1 self-center mr-2">
                    <i class='bx bx-filter-alt text-xs'></i> Filter Periode
                </p>
                <div class="flex items-end gap-2 flex-wrap">
                    <div>
                        <label class="text-[8px] font-bold text-natural-400 uppercase ml-0.5">Dari</label>
                        <input type="date" name="start_date" value="{{ request('start_date') }}"
                               class="bg-natural-50 border-none rounded-lg text-[11px] py-1.5 px-2.5 focus:ring-2 focus:ring-brand-500/20 text-natural-700 font-medium">
                    </div>
                    <div>
                        <label class="text-[8px] font-bold text-natural-400 uppercase ml-0.5">Sampai</label>
                        <input type="date" name="end_date" value="{{ request('end_date') }}"
                               class="bg-natural-50 border-none rounded-lg text-[11px] py-1.5 px-2.5 focus:ring-2 focus:ring-brand-500/20 text-natural-700 font-medium">
                    </div>
                    <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[11px] py-1.5 px-3 rounded-lg transition-all">
                        Filter
                    </button>
                    @if(request('start_date') || request('end_date'))
                        <a href="{{ route('reports.index') }}" class="text-[9px] font-bold text-natural-400 hover:text-rose-500 transition-colors self-center">Reset</a>
                    @endif
                </div>
                <div class="ml-auto text-right self-center">
                    @if(request('start_date') && request('end_date'))
                        <p class="text-[9px] text-brand-500 font-bold">{{ request('start_date') }} s/d {{ request('end_date') }}</p>
                    @else
                        <p class="text-[9px] text-natural-400 font-medium">Bulan {{ now()->translatedFormat('F Y') }}</p>
                    @endif
                </div>
            </div>
        </form>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
            ...
            <div class="bg-white p-3 rounded-2xl border border-natural-100 shadow-sm flex items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    @else
                        <p class="text-[9px] text-rose-600 mt-0.5 font-bold">↓ {{ number_format($growthPendapatan, 1) }}% dari bln lalu</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white p-3 rounded-2xl border border-natural-100 shadow-sm flex items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    @else
                        <p class="text-[9px] text-rose-600 mt-0.5 font-bold">↓ {{ number_format($growthModal, 1) }}% dari bln lalu</p>
                    @endif
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
            ...
            <div class="bg-white p-3 rounded-2xl border border-violet-100 shadow-sm flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center text-lg shrink-0">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    @else
                        <p class="text-[9px] text-rose-600 mt-0.5 font-bold">↓ {{ number_format($growthService, 1) }}% dari bln lalu</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white p-3 rounded-2xl border border-cyan-100 shadow-sm flex items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    @else
                        <p class="text-[9px] text-rose-600 mt-0.5 font-bold">↓ {{ number_format($growthRental, 1) }}% dari bln lalu</p>
                    @endif
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 shrink-0">
                    ...
                    <table class="w-full text-left border-separate border-spacing-0">
                        <thead>
                            <tr class="bg-white text-natural-500 text-[10px] uppercase border-b border-natural-100">
                                <th class="px-4 py-2 font-bold">Metrik Keuangan</th>
                                <th class="px-4 py-2 font-bold text-right">Bulan Lalu</th>
                                <th class="px-4 py-2 font-bold text-right">Bulan Ini</th>
                                <th class="px-4 py-2 font-bold text-right">Tren</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-natural-50 text-[11px]">
                            {{-- Penjualan --}}
                            <tr class="bg-emerald-50/30">
                                <td colspan="4" class="px-4 pt-2 pb-1 text-[9px] font-black text-emerald-600 uppercase tracking-widest">
                                    <i class='bx bx-cart-alt mr-1'></i> Penjualan
                                </td>
                            </tr>
                            <tr class="hover:bg-natural-50/30 transition-colors">
                                <td class="px-4 py-1.5 font-semibold text-natural-800 pl-6">Pendapatan Penjualan</td>
                                <td class="px-4 py-1.5 text-right text-natural-500">Rp {{ number_format($pmPendapatan, 0, ',', '.') }}</td>
                                <td class="px-4 py-1.5 text-right font-bold text-natural-800">Rp {{ number_format($cmPendapatan, 0, ',', '.') }}</td>
                            ...
                            <tr class="hover:bg-natural-50/30 transition-colors">
                                <td class="px-4 py-1.5 font-semibold text-natural-800 pl-6">Modal Stok</td>
                                <td class="px-4 py-1.5 text-right text-natural-500">Rp {{ number_format($pmModal, 0, ',', '.') }}</td>
                                <td class="px-4 py-1.5 text-right font-bold text-natural-800">Rp {{ number_format($cmModal, 0, ',', '.') }}</td>
                                <td class="px-4 py-1.5 text-right font-black">
                                    @if($growthModal >= 0)
                            ...
                            <tr class="bg-violet-50/30">
                                <td colspan="4" class="px-4 pt-2 pb-1 text-[9px] font-black text-violet-600 uppercase tracking-widest">
                                    <i class='bx bx-wrench mr-1'></i> Service
                                </td>
                            </tr>
                            <tr class="hover:bg-natural-50/30 transition-colors">
                                <td class="px-4 py-1.5 font-semibold text-natural-800 pl-6">Pendapatan Service</td>
                                <td class="px-4 py-1.5 text-right text-natural-500">Rp {{ number_format($pmService, 0, ',', '.') }}</td>
                                <td class="px-4 py-1.5 text-right font-bold text-natural-800">Rp {{ number_format($cmService, 0, ',', '.') }}</td>
                                <td class="px-4 py-1.5 text-right font-black">
                                    @if($growthService >= 0)
                            ...
                            <tr class="bg-cyan-50/30">
                                <td colspan="4" class="px-4 pt-2 pb-1 text-[9px] font-black text-cyan-600 uppercase tracking-widest">
                                    <i class='bx bx-laptop mr-1'></i> Sewa Laptop
                                </td>
                            </tr>
                            <tr class="hover:bg-natural-50/30 transition-colors">
                                <td class="px-4 py-1.5 font-semibold text-natural-800 pl-6">Pendapatan Sewa</td>
                                <td class="px-4 py-1.5 text-right text-natural-500">Rp {{ number_format($pmRental, 0, ',', '.') }}</td>
                                <td class="px-4 py-1.5 text-right font-bold text-natural-800">Rp {{ number_format($cmRental, 0, ',', '.') }}</td>
                                <td class="px-4 py-1.5 text-right font-black">
                                    @if($growthRental >= 0)
                            ...
                            <tr class="bg-natural-900 text-white">
                                <td class="px-4 py-2 font-black text-[11px]">Total Seluruh Pendapatan</td>
                                <td class="px-4 py-2 text-right font-bold text-natural-300">Rp {{ number_format($totalPmAll, 0, ',', '.') }}</td>
                                <td class="px-4 py-2 text-right font-black">Rp {{ number_format($totalCmAll, 0, ',', '.') }}</td>
                                <td class="px-4 py-2 text-right font-black">
                                    @if($growthAll >= 0)
            ...
            <div class="lg:col-span-1 bg-white rounded-3xl shadow-sm border border-natural-100/50 overflow-hidden">
                <div class="px-4 py-2 border-b border-natural-100 bg-natural-50/50 flex items-center gap-2">
                    <i class='bx bx-trophy text-amber-500 text-sm'></i>
                    <h3 class="text-xs font-bold text-natural-800">Produk Terlaris <span class="text-[9px] text-natural-400 font-medium">(Bulan Ini)</span></h3>
                </div>
                <div class="p-3 space-y-2">
                    @forelse($topProducts as $item)
                        <div class="flex items-center justify-between gap-2">
                            <div class="flex items-center gap-2 min-w-0">
                                <div class="w-6 h-6 rounded-lg bg-brand-50 text-brand-600 flex items-center justify-center text-[10px] font-black shrink-0">
                                    {{ $loop->iteration }}
                                </div>
                                <p class="text-[11px] font-semibold text-natural-800 truncate">{{ $item['name'] ?: 'Produk Tanpa Nama' }}</p>
                            </div>
                            <span class="shrink-0 px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded text-[9px] font-black">{{ $item['count'] }} terjual</span>
                        </div>
                    @empty
                        <p class="text-[11px] text-natural-400 italic text-center py-3">Belum ada penjualan bulan ini.</p>
                    @endforelse
                </div>

                {{-- Mini stats service & sewa bulan ini --}}
                <div class="border-t border-natural-100 px-3 py-2.5 space-y-1.5">
                    <p class="text-[8px] font-black text-natural-400 uppercase tracking-wider">Transaksi Bulan Ini</p>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-1.5">
                            <i class='bx bx-wrench text-violet-500 text-xs'></i>
                            <span class="text-[11px] text-natural-600 font-medium">Service</span>
                        </div>
                        <span class="text-[11px] font-black text-violet-600">{{ $services->count() }} order</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-1.5">
                            <i class='bx bx-laptop text-cyan-500 text-xs'></i>
                            <span class="text-[11px] text-natural-600 font-medium">Sewa Laptop</span>
                        </div>
                        <span class="text-[11px] font-black text-cyan-600">{{ $rentals->count() }} unit</span>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-natural-100/50 overflow-hidden flex-grow flex flex-col">
            ...
            {{-- Tab Navigation --}}
            <div class="px-4 py-2 border-b border-natural-100 bg-natural-50/50 flex items-center gap-3 flex-wrap">
                <i class='bx bx-receipt text-natural-500 text-sm'></i>
                <h3 class="text-xs font-bold text-natural-800 mr-2">Rincian Transaksi
                    @if(request('start_date') && request('end_date'))
                        <span class="text-[9px] text-brand-500 font-medium">({{ request('start_date') }} s/d {{ request('end_date') }})</span>
                    @else
                        <span class="text-[9px] text-natural-400 font-medium">(Bulan {{ now()->translatedFormat('F Y') }})</span>
                    @endif
                </h3>

                <div class="flex items-center gap-1 ml-auto">
                    <button onclick="switchTab('semua')" id="tab-semua"
                            class="tab-btn active-tab px-2.5 py-1 rounded-lg text-[9px] font-black transition-all">
                        Semua
                        <span class="ml-1 px-1 py-0.5 rounded bg-natural-100 text-natural-600 text-[8px]">
                            {{ $sales->count() + $services->count() + $rentals->count() }}
                        </span>
                    </button>
                    <button onclick="switchTab('penjualan')" id="tab-penjualan"
                            class="tab-btn px-2.5 py-1 rounded-lg text-[9px] font-black transition-all">
                        Penjualan
                        <span class="ml-1 px-1 py-0.5 rounded bg-emerald-50 text-emerald-700 text-[8px]">{{ $sales->count() }}</span>
                    </button>
                    <button onclick="switchTab('service')" id="tab-service"
                            class="tab-btn px-2.5 py-1 rounded-lg text-[9px] font-black transition-all">
                        Service
                        <span class="ml-1 px-1 py-0.5 rounded bg-violet-50 text-violet-700 text-[8px]">{{ $services->count() }}</span>
                    </button>
                    <button onclick="switchTab('sewa')" id="tab-sewa"
                            class="tab-btn px-2.5 py-1 rounded-lg text-[9px] font-black transition-all">
                        Sewa
                        <span class="ml-1 px-1 py-0.5 rounded bg-cyan-50 text-cyan-700 text-[8px]">{{ $rentals->count() }}</span>
                    </button>
                </div>
            </div>
                ...
                <table class="w-full text-left border-separate border-spacing-0">
                    <thead>
                        <tr class="bg-natural-50/50 border-b border-natural-100">
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider">Tipe</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider">Nomor &amp; Tanggal</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider">Informasi</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider whitespace-nowrap">Pendapatan</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider text-right whitespace-nowrap">Status</th>
                        </tr>
                    </thead>
            ...
            <div id="content-penjualan" class="tab-content hidden overflow-x-auto">
                <table class="w-full text-left border-separate border-spacing-0">
                    <thead>
                        <tr class="bg-natural-50/50 border-b border-natural-100">
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider">Faktur &amp; Tanggal</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider">Informasi Produk</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider">Metode</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider whitespace-nowrap">Pendapatan</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider whitespace-nowrap">Modal Item</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-natural-500 uppercase tracking-wider text-right whitespace-nowrap">Laba Bersih</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-natural-50">
                ...
                <table class="w-full text-left border-separate border-spacing-0">
                    <thead>
                        <tr class="bg-violet-50/50 border-b border-violet-100">
                            <th class="px-4 py-2 text-[10px] font-bold text-violet-500 uppercase tracking-wider">No. Servis &amp; Tanggal</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-violet-500 uppercase tracking-wider">Perangkat &amp; Keluhan</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-violet-500 uppercase tracking-wider">Pelanggan</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-violet-500 uppercase tracking-wider whitespace-nowrap">Total Tagihan</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-violet-500 uppercase tracking-wider whitespace-nowrap">Biaya Jasa</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-violet-500 uppercase tracking-wider text-right whitespace-nowrap">Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-natural-50">
                ...
                <table class="w-full text-left border-separate border-spacing-0">
                    <thead>
                        <tr class="bg-cyan-50/50 border-b border-cyan-100">
                            <th class="px-4 py-2 text-[10px] font-bold text-cyan-600 uppercase tracking-wider">No. Sewa &amp; Tanggal</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-cyan-600 uppercase tracking-wider">Laptop</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-cyan-600 uppercase tracking-wider">Pelanggan</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-cyan-600 uppercase tracking-wider whitespace-nowrap">Durasi Sewa</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-cyan-600 uppercase tracking-wider whitespace-nowrap">Harga/Hari</th>
                            <th class="px-4 py-2 text-[10px] font-bold text-cyan-600 uppercase tracking-wider text-right whitespace-nowrap">Total &amp; Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-natural-50">
