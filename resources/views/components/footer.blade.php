<footer class="bg-white border-t border-gray-200 mt-auto pt-6 md:pt-10 pb-6 max-md:pb-24">
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-8 items-start">
            ...
            <div>
                <h4 class="font-bold text-gray-800 mb-2 md:mb-3 font-montserrat h-6 flex items-center max-md:text-[1.1rem]">Hubungi Kami</h4>
                <ul class="flex flex-col gap-2 md:gap-2.5 text-sm max-md:text-[0.825rem] text-gray-600 max-md:leading-[1.15]">
                    <li class="flex items-start gap-3">
                        <i class='bx bx-map text-lg max-md:text-[0.95rem] text-brand-500 mt-[2px] flex-shrink-0'></i>
                        <span class="leading-snug">{!! $settings->alamat ?? 'Villa Mutiara 1 Sektor 2 BLOK i-18 No.03<br>Tanah Sereal, Bogor 16168' !!}</span>
                    </li>
                    <li class="flex items-start gap-3">
                        <i class='bx bx-envelope text-lg max-md:text-[0.95rem] text-brand-500 mt-[2px] flex-shrink-0'></i>
                        <a href="mailto:{{ strip_tags($settings->email ?? 'sales@lktech.online') }}" class="hover:text-brand-600 transition-colors leading-snug">{{ strip_tags($settings->email ?? 'sales@lktech.online') }}</a>
                    </li>
                    <li class="flex items-start gap-3">
                        <i class='bx bxl-whatsapp text-lg max-md:text-[0.95rem] text-brand-500 mt-[2px] flex-shrink-0'></i>
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $settings->telepon ?? '628567354046') }}" target="_blank" class="hover:text-brand-600 transition-colors leading-snug">{{ $settings->telepon ?? '+62 856-7354-046' }}</a>
                    </li>
                    <li class="flex items-start gap-3">
                        <i class='bx bx-time-five text-lg max-md:text-[0.95rem] text-brand-500 mt-[2px] flex-shrink-0'></i>
                        <span class="leading-snug">{{ $settings->jam_operasional ?? 'Senin - Sabtu: 09:00 - 17:00' }}</span>
                    </li>
                </ul>

                <!-- Ikuti Kami -->
                <h4 class="font-bold text-gray-800 mt-4 md:mt-5 mb-2 font-montserrat h-6 flex items-center max-md:text-[1.1rem]">Ikuti Kami</h4>
