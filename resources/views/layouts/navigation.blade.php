    ...
    <div class="flex flex-col flex-1 bg-white rounded-3xl shadow-sm border border-natural-100/50 overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto py-2 px-3 space-y-0.5 custom-scrollbar transition-all duration-300">
            ...
            <a href="{{ route('google-reviews.index') }}" 
               class="flex items-center gap-3 py-1.5 rounded-xl transition-all duration-200 group {{ request()->routeIs('google-reviews.*') ? 'bg-yellow-50 text-yellow-700 font-bold mr-2' : 'text-natural-600 hover:bg-natural-50 hover:text-natural-900 font-medium' }}"
               :class="sidebarOpen ? 'px-5 justify-start' : 'px-0 justify-center'" title="Ulasan Google">
            ...
            <a href="{{ route('promo.edit') }}" 
               ...
               :class="sidebarOpen ? 'px-5 justify-start' : 'px-0 justify-center'" title="Banner & Produk Promo">
                ...
                <span x-show="sidebarOpen" x-transition.opacity class="whitespace-nowrap text-[13px]">Banner & Produk Promo</span>
            </a>
            @endif
            @endif
