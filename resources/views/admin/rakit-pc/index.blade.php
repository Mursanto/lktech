    ...
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <h2 class="font-bold text-base text-natural-900 leading-tight">
                Manajemen Paket Rakit PC
            </h2>
            <div class="flex items-center gap-3">
                <a href="{{ route('rakit-pc-admin.create') }}" class="bg-brand-600 hover:bg-brand-700 text-white px-3 py-1.5 rounded-lg font-bold text-[11px] shadow flex items-center gap-1.5 transition-colors">
                    ...
                    <table class="w-full text-left text-sm whitespace-nowrap">
                        <thead class="bg-gray-50 text-gray-400 font-bold text-[10px] uppercase tracking-wider sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                <th class="px-4 py-2 border-b">Harga Estimasi</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-4 py-2">
                                    @if($package->foto)
                                        <img src="{{ Storage::url($package->foto) }}" alt="Foto" class="h-8 w-12 object-contain bg-white rounded-md border border-gray-200 p-0.5">
                                    @else
                                        <div class="h-8 w-12 bg-gray-100 rounded-md flex items-center justify-center border border-gray-200 text-gray-400">
                                ...
                                <td class="px-4 py-2 text-[11px] text-gray-800 font-bold">
                                    Rp {{ number_format($package->harga_estimasi, 0, ',', '.') }}
                                </td>
                                ...
                                <td class="px-4 py-2 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('rakit-pc-admin.edit', $package) }}" class="text-blue-600 hover:text-blue-800 transition-colors p-1 bg-blue-50 hover:bg-blue-100 rounded transition-all" title="Edit">
                                            <i class='bx bx-edit-alt text-sm'></i>
                                        </a>
                                        <form action="{{ route('rakit-pc-admin.destroy', $package) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus paket ini?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="5" class="px-4 py-12 text-center text-gray-500">
                                    <i class='bx bx-desktop text-4xl mb-2 text-gray-300'></i>
                                    <p>Belum ada paket Rakit PC.</p>
                                    <a href="{{ route('rakit-pc-admin.create') }}" class="text-brand-600 hover:underline text-sm font-bold mt-2 inline-block">Tambah Paket Baru</a>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
