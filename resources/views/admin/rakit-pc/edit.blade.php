<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <form action="{{ route('rakit-pc-admin.update', $package) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div class="md:col-span-2 space-y-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="harga_estimasi" id="harga_estimasi" value="{{ old('harga_estimasi', (int)$package->harga_estimasi) }}" required min="0" step="1000"
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" id="deskripsi" rows="3"
                                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
                                          placeholder="Contoh: Lancar bermain game e-sports & editing video 1080p...">{{ old('deskripsi', $package->deskripsi) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="spesifikasi_singkat" id="spesifikasi_singkat" rows="5"
                                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
                                          placeholder="Contoh: CPU: Core i5 / Ryzen 5&#10;RAM: 16GB DDR4&#10;Storage: 512GB SSD NVMe">{{ old('spesifikasi_singkat', $package->spesifikasi_singkat) }}</textarea>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-gray-50 p-5 rounded-xl border border-gray-100">
                                ...
                                <label class="flex items-center gap-3 cursor-pointer mb-6">
                                    ...
                                    <span class="text-sm font-medium {{ old('is_active', $package->is_active) ? 'text-gray-700' : 'text-gray-400' }} font-bold toggle-label">
                                        {{ old('is_active', $package->is_active) ? 'Aktif (Tampil)' : 'Nonaktif (Sembunyikan)' }}
                                    </span>
                                </label>
                            ...
                            <div class="bg-gray-50 p-5 rounded-xl border border-gray-100">
                                ...
                                <label for="foto" class="relative cursor-pointer block mt-1 px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg bg-white group overflow-hidden hover:border-brand-500 transition-colors text-center" id="foto-preview-container">
                                    ...
                                    <div class="space-y-1 relative z-10 transition-opacity duration-300 {{ $package->foto ? 'opacity-0' : '' }}" id="foto-upload-text">
    ...
    <script>
        ...
        document.getElementById('foto-preview-container').addEventListener('mouseenter', function() {
            document.getElementById('foto-hover-overlay').classList.remove('opacity-0');
        });
        document.getElementById('foto-preview-container').addEventListener('mouseleave', function() {
            document.getElementById('foto-hover-overlay').classList.add('opacity-0');
        });
    </script>
</x-app-layout>
